...
export default function ResourceCard({
  ...
  const theme = isDatabase
    ? {
        ...
        shadowHover: "hover:shadow-slate-900/10 dark:hover:shadow-none",
        gradientStart: "from-primary/5",
        gradientEnd: "to-primary/5",
        iconBoxGradientStart: "bg-slate-100 dark:bg-neutral-950",
  ...
  return (
    <div
      className={`group relative overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm shadow-slate-900/5 backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl dark:border-neutral-800 dark:bg-neutral-950 dark:shadow-none ${theme.borderHover} ${theme.shadowHover}`}
    >
      {/* Gradient Glow */}
      <div
        className={`absolute inset-0 bg-linear-to-br ${theme.gradientStart} via-transparent ${theme.gradientEnd} opacity-0 group-hover:opacity-100 transition-opacity duration-500 dark:hidden`}
      />

      <Link href={href} className="block p-6 relative z-10">
        <div className="flex items-start justify-between mb-6">
          <div
            className={`w-12 h-12 rounded-xl ${theme.iconBoxGradientStart} border border-slate-200 dark:border-neutral-800 flex items-center justify-center ${theme.iconBoxHoverStart} ${theme.iconBoxHoverEnd} ${theme.iconBoxBorderHover} transition-all duration-300 shadow-sm shadow-slate-900/5 dark:shadow-none`}
          >
            <theme.Icon
